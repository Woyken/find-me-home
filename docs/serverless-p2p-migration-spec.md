...
The saved bookmarklet is a short loader (well under 1,000 characters, so
browsers do not truncate it when it is pasted as a bookmark) generated by the
app with the current Pages base URL. It injects a script tag for the scraper,
which Pages serves as `aruodas-bookmarklet.js` with `Access-Control-Allow-Origin:
*`, falling back to fetch and eval if the script element fails. Scraper updates
therefore ship without users re-saving the bookmark. Aruodas.lt sends no
Content-Security-Policy; if it ever does, the loader alerts and the inlined
scraper must be revisited. It opens no popup and posts no form. Megabyte-sized imports are
