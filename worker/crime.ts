...
const BK_IDS = [
  129, 130, 131, 135, 149, 178, 180, 182, 187, 259, 260, 261, 262, 263, 264, 265, 266, 267, 268,
  284,
]
const VIOLENT_ARTICLES = new Set([129, 130, 131, 135, 149, 150, 151, 180])

export const handleCrimeRequest = async (request: Request, options: WorkerOptions) => {
  ...
  try {
    const [x, y] = proj4('EPSG:4326', 'EPSG:3346', [point.longitude, point.latitude])
    const now = options.now?.() ?? new Date()
    const from = new Date(now)
    from.setUTCFullYear(from.getUTCFullYear() - years)
    const dateFrom = from.toISOString().slice(0, 10)
    const dateTo = now.toISOString().slice(0, 10)
    const ring: Array<[number, number]> = Array.from({ length: 32 }, (_, index) => {
    ...
    try {
      ...
      throw new Error(`IRD returned non-JSON body (${text.length} bytes): ${text.slice(0, 120)}`)
    }
    if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed))
      throw new Error(`IRD returned a non-object JSON body: ${text.slice(0, 120)}`)
    ...
    for (const row of bare) {
      if (!Array.isArray(row) || row.length < 4)
        throw new Error(`IRD row has unexpected shape: ${JSON.stringify(row).slice(0, 120)}`)
      const px = row[1]
      const py = row[2]
      if (typeof px !== 'number' || typeof py !== 'number')
        throw new Error(`IRD row has non-numeric coordinates: ${JSON.stringify(row).slice(0, 120)}`)
    ...
    return Response.json({ error: 'IRD unavailable', reason }, { status: 502, headers: cors })
  }
}
