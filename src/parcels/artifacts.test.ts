...
import { PARCEL_MUNICIPALITIES, buildParcelAssets, validateParcelAssets } from './artifacts'
...
describe('Registered Parcel artifacts', () => {
  it('makes every parcel reachable by all intersecting cells and complete numbers', async () => {
    ...
    const prefix = JSON.parse(
      gunzipSync(
        await readFile(path.join(outputDirectory, manifest.prefixes['1300'].path)),
      ).toString(),
    ...
    await expect(validateParcelAssets(outputDirectory)).resolves.toEqual(manifest)

    const prefixPath = path.join(outputDirectory, manifest.prefixes['1300'].path)
    const incomplete = JSON.parse(gunzipSync(await readFile(prefixPath)).toString()) as {
      references: Array<[string, Array<[number, string, string]>]>
    }
    ...
    manifest.prefixes['1300'].sha256 = createHash('sha256').update(incompleteBytes).digest('hex')
    await writeFile(path.join(outputDirectory, 'manifest.json'), JSON.stringify(manifest))
    await expect(validateParcelAssets(outputDirectory)).rejects.toThrow(/reachability/i)
  })
  ...
  it('rejects corrupted and oversized publication assets', async () => {
    ...
    await expect(validateParcelAssets(outputDirectory)).rejects.toThrow(/checksum|corrupt|gzip/i)
    await buildParcelAssets(sources(), outputDirectory)
    await expect(validateParcelAssets(outputDirectory, { maxTotalBytes: 1 })).rejects.toThrow(
      /size limit/i,
    )
    ...
    await expect(validateParcelAssets(outputDirectory)).rejects.toThrow(/dataset version mismatch/i)
  })
})
