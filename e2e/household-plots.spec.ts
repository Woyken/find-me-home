...
test('creates a search, persists it, and handles invalid and hash invitations', async ({
  ...
  await expect(
    page.getByRole('heading', { name: 'Our home search' }),
  ).toBeVisible()
  await page.reload()
  await expect(
    page.getByRole('heading', { name: 'Our home search' }),
  ...
  await expect(
    page.getByRole('heading', { name: 'Willow search' }),
  ).toBeVisible()
...
test('manages settings and navigates empty plots and not-found pages', async ({
  page,
}) => {
  ...
  await page.evaluate(() =>
    window.__FMH_E2E__?.createHousehold('Second search'),
  )
  await expect(
  ...
  await page
    .getByRole('button', { name: 'Remove this search from this device' })
    .click()
  await expect(
    page.getByRole('heading', { name: 'Second search' }),
  ).toBeVisible()
...
test('sorts, filters, maps, and plans populated located and unlocated plots', async ({
  ...
  await expect(plots.savedPlots.getByRole('article').first()).toContainText(
    'Cheap unlocated',
  )
  ...
  await expect(
    plots.row('Middle located').row.getByRole('button'),
  ).toContainText('Going to see')
  await plots
    .row('Middle located')
  ...
  await expect(plots.row('Cheap unlocated').row).toContainText(
    'no location yet',
  )
